...
class VectorStore:
    ...
    def search_similar_patterns(
        ...
        results = self.db.execute(
            sql, {"embedding": str(query_embedding), "threshold": threshold, "limit": top_k}
        ).fetchall()
    ...
    def add_pattern_embedding(self, pattern_id: str, description: str, embedding_model):
        ...
        self.db.execute(sql, {"embedding": str(embedding), "pattern_id": pattern_id})
        self.db.commit()

    def search_similar_blacklist(
        self, query_text: str, embedding_model, top_k: int = 5, threshold: float = 0.8
    ) -> list[dict]:
        """
