...
class ExcelImporter:
    ...
    def import_to_blacklist(
        cls, db: Session, file_path: str, source: str = "excel_import", risk_score: float = 0.95
    ) -> dict:
        """
        ...
        for idx, row in df.iterrows():
            try:
                ...
                existing = (
                    db.query(Blacklist)
                    .filter(
                        Blacklist.entity_value == stk, Blacklist.entity_type == "account", Blacklist.is_active.is_(True)
                    )
                    .first()
                )
                ...
                blacklist_entry = Blacklist(
                    ...
                    evidence={"ten": ten, "ngan_hang": ngan_hang, "imported_from": file_path, "row_index": int(idx)},
                    is_active=True,
                )
